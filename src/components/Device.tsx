...
export const Device = ({ device, selected, id }: { device: DeviceInterface; selected: boolean; id: number }) => {
  ...
  return (
    <>
      <Link
        to={`/drive/${id}`}
        className="device flex items-center justify-between mb-6 cursor-pointer transition-all hover:opacity-70"
      >
        ...
        <div className="right flex items-center">
          <svg
            ...
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M5 19V17H19V19H5ZM5.35 15L12 5L18.65 15H5.35Z" fill="var(--icon-color)" />
          </svg>
          <svg
      ...
      <Tooltip className="tooltip z-[999]" id={randomTooltipID} opacity={'100%'}>
        <div className="p-2">
          <h1 className="text-xl mb-4 text-white">{device.name}</h1>
          <p className="opacity-50 mb-4 text-white">

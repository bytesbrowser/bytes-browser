      ...
      >
        <div className="animate__animated animate__fadeIn animate__faster">
          <div className="modal-top h-[250px] flex justify-center items-center flex-col">
            ...
            <img src="/bytes_logo.png" className="w-[150px]" />
          ...
          <div className="py-12 px-8">
            <form className="flex flex-col justify-center" onSubmit={onFeedback}>
              ...
              <input
                type="email"
                required
                className={`text-sm p-3 rounded-md bg-sidebar border border-light-border transition-all outline-none focus:border-gray-400 w-full`}
                placeholder="you@example.com"
                value={feedbackForm.email}
                onChange={(e) => setFeedbackForm({ ...feedbackForm, email: e.target.value })}
              />
              <p className="mb-4 opacity-50 text-md mt-12">Category (Optional)</p>
              <input
                type="text"
                placeholder="Ideas, Bugs, etc."
                className={`text-sm w-full p-3 rounded-md bg-sidebar border border-light-border transition-all outline-none focus:border-gray-400`}
                value={feedbackForm.category}
                onChange={(e) => setFeedbackForm({ ...feedbackForm, category: e.target.value })}
              />
              <p className="mb-4 opacity-50 text-md mt-12">Feedback</p>
              <textarea
                required
                placeholder="I really like how..."
                value={feedbackForm.feedback}
                onChange={(e) => setFeedbackForm({ ...feedbackForm, feedback: e.target.value })}
                className={`text-sm resize-none p-3 rounded-md bg-sidebar border border-light-border transition-all outline-none focus:border-gray-400 w-full`}
              ></textarea>
              {submittingFeedback ? (
